class CategoryWatchersController < ApplicationController
  ...
  before_action :authorize_cw, :only => [:add, :index]
	...
	def index
	  ...
	  @users = @project.users.order(:firstname, :lastname)  # Ordenar usuarios por nombre

	  categories_tmp = IssueCategory.where(project_id: @project.id).order(:name)  # Ordenar categorías por nombre
	  ...
	  categories_tmp.each do |category|
		...
		if cw.nil?
		  cw = CategoryWatcher.create(category_id: category.id, watchers: " ")
		end      

		ids = cw.watchers.split(',')
		cw.selected_users = User.where(id: ids).order(:firstname, :lastname).map { |user| { id: user.id.to_i, name: user.name } }
